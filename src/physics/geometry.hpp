...
struct TFace {
	...
	// ќбновл€ет единичные касательный и нормальный векторы
	void updateUnitVectors() {              // normal
		tangent = end - beg;                // ^
		tangent.normalize();                // |
		// Ћева€ нормаль                       |
		normal = { tangent.y, -tangent.x }; // O----> tangent    Ќќ! ось y направлена вниз!
	}
	...
	// ƒл€ этого используем свойство векторного произведени€ [tangent, beg_pnt]
